  ...
  secrets: {
    providers: {
      passwd: {
        ...
        args: ["-y", "@passwd/passwd-cli@1.5.7", "resolve"],
...
Store your API keys as secrets in passwd.team, then use their IDs in the `id` field. Run `npx @passwd/passwd-cli@1.5.7 list` to find them.
...
```bash
PASSWD_ORIGIN=https://your-deployment.passwd.team npx -y @passwd/passwd-agent-cli@1.5.7 login
```
...
For multiple deployments, log in to each origin separately (`PASSWD_ORIGIN=... npx @passwd/passwd-agent-cli@1.5.7 login`). The agent can then switch with `--env` — see the Multi-environment section in the skill above.
...
```json
{
  ...
  "command": "npx",
  "args": [
    "-y", "@passwd/passwd-agent-cli@1.5.7", "mcp-wrap",
...
```json
{
  "mcpServers": {
    "passwd": {
      "command": "npx",
      "args": ["-y", "@passwd/passwd-mcp@1.5.7"],
      "env": {
        "PASSWD_ORIGIN": "https://your-deployment.passwd.team"
      }
    }
...
npx @passwd/passwd-agent-cli@1.5.7 login
npx @passwd/passwd-agent-cli@1.5.7 list
npx @passwd/passwd-agent-cli@1.5.7 exec --inject DB_PASS=SECRET_ID:password -- psql -h host -U app
```
...
```bash
export PASSWD_ORIGIN=https://your-deployment.passwd.team
npx @passwd/passwd-cli@1.5.7 login
npx @passwd/passwd-cli@1.5.7 list
npx @passwd/passwd-cli@1.5.7 --help
```

For multiple deployments, log in to each origin separately, then use `--env` to switch:

```bash
PASSWD_ORIGIN=https://acme.passwd.team npx @passwd/passwd-cli@1.5.7 login
PASSWD_ORIGIN=https://initech.passwd.team npx @passwd/passwd-cli@1.5.7 login
npx @passwd/passwd-cli@1.5.7 list --env acme
```
